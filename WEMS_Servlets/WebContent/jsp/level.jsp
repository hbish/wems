...
<meta http-equiv="Refresh" content="6,URL=http://220.244.243.51:8080/WEMS_Servlets/jsp/level.jsp?building=<%=building%>&level=<%=level%>">
...
function OpenLogoutScriptlet() {
	window.location = "http://220.244.243.51:8080/WEMS_Servlets/Logout?user-name=<%=loginBean.getUserNameByIP(request.getRemoteAddr())%>";
		...
		<ul id="breadcrumb">
            ...
            <li><a href="#" title="Level XXX">Level <%=level%></a></li>
            <!-- InstanceEndEditable -->
		</ul>
...
<article class="container_12">
        ...
        <h1>Level <%=level%> of Building  <%=building%></h1>
        
		<!-- List of Levels -->
        <section class="grid_12">
			<div class="block-border"><form class="block-content form" id="table_form" method="post" action="">
				<h1>Rooms on Level  <%=level%></h1>
				...
				<table class="table sortable no-margin" cellspacing="0" width="100%">
					...
					<thead>
						...
						<th scope="col"><span class="column-sort">
							...
							<th scope="col">Number of Active Alerts</th>
                            <th scope="col" class="table-actions">Actions</th>
					</thead>
					
					<tbody>
						<%//List<String> roomNames = locBean.getRooms(building, level); %>
						<%//List<String> roomDC = locBean.getRoomsDC(building, level);%>
						<%//List<Integer> roomAlerts = locBean.getRoomsAlerts(building, level);%>
						<%List<SensordataRoomDataClass> rooms = locBean.getRoomDetails(building, level); %>
						<%for (int i =0; i<rooms.size();i++){ %>
						<tr>
							<td><%=rooms.get(i).getName()%></td>
							<td>
							<%if(rooms.get(i).getConnected()){%>
							...
							<td><%=rooms.get(i).getPowerUsage()%></td>
							<td><%=rooms.get(i).getNumAlerts()%></td>
							<td class="table-actions">
								<a href="room.jsp?building=<%=building%>&level=<%=level%>&room=<%=rooms.get(i).getName()%>" title="View" class="with-tip"><img src="images/icons/fugue/magnifier.png" width="16" height="16"></a>
							</td>
						</tr>
                        <%}%>
