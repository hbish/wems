    ...
    <script type="text/javascript">
		...
		function deleteUserGroup(userGroupID)
		{
			$.modal({
				...
				'<form class="form with-margin" name="delete-user-group-form" id="delete-user-group-form" method="post" action="delete_user_group.jsp">'+
...
function OpenLogoutScriptlet() {
	window.location = "http://220.244.243.51:8080/WEMS_Servlets/Logout?user-name=<%=loginBean.getUserNameByIP(request.getRemoteAddr())%>";
		...
		<ul id="breadcrumb">
            ...
            <li><a href="#" title="User Groups">User Groups</a></li>
            <!-- InstanceEndEditable -->
		</ul>
        ...
        <section class="grid_12">
            ...
            String success = "";
					if(request.getParameter("success") != null){
						...
						if (success.equals("1")){
						out.println("<FONT COLOR=\"green\">User Group successfully deleted.</FONT>");
					}
		%>
				<table class="table sortable no-margin" cellspacing="0" width="100%">
					...
					<tbody>
						...
						<% List<String> userGroups = configBean.getAllUserGroupNames();
					List<Integer> userGroupsId = configBean.getAllUserGroupId();
				   	Iterator<String> itr = userGroups.iterator();
				   	Iterator<Integer> itr2 = userGroupsId.iterator();%>
				   	...
				   	<%while(itr.hasNext()&&itr2.hasNext()){
						...
						<tr>
							...
							<td class="table-actions">
								<a href="edit_user_group.jsp?group=<%=id%>" title="Edit" class="with-tip"><img src="images/icons/fugue/pencil.png" width="16" height="16"></a>
								<a href="#" title="Delete" class="with-tip" onClick="OpenDeleteScriptlet(<%=id%>);"><img src="images/icons/fugue/cross-circle.png" width="16" height="16"></a>
							</td>
						</tr>
                       <%}%>
					...
					<SCRIPT language = JavaScript>
function OpenDeleteScriptlet(id) {
	shouldDelete=confirm('Do you really want to delete the usergroup?');
	  if(shouldDelete){
		  //TODO FIX THIS!
		window.location = "http://220.244.243.51:8080/WEMS_Servlets/DeleteUserGroup?userGroupID="+id;
	}
}
</SCRIPT>
			</form></div>
